...
export const api = {
  ...
  // Questions
  nextQuestion: (body: { subject: string; track: string; difficulty: string; count?: number; source?: string; year?: number; mode?: string; type?: string; exclude_ids?: string[] }) =>
    request<{ question: unknown }>("/questions/next", { method: "POST", body: JSON.stringify(body) }),

  guestNextQuestion: (body: { subject: string; track: string; difficulty: string; source?: string; year?: number; mode?: string; type?: string; exclude_ids?: string[] }) =>
